...
void PointCloudCorrespondenceDisplay::processMessage(const icp_visualization_rviz_plugin::iCPCorrespondenceConstPtr& msg){
    if(msg->index_curr.size() != msg->index_prev.size()){
    ...
    if (!context_->getFrameManager()->getTransform(msg->cloud_prev.header, position_s, orientation_s)) {
        ROS_ERROR("Error transforming from frame '%s' to frame '%s'", msg->cloud_prev.header.frame_id.c_str(),
    ...
    auto points_s = transform_pointcloud(msg->cloud_prev, transform_s, msg->index_prev);

    Ogre::Vector3 position_e;
    Ogre::Quaternion orientation_e;
    if (!context_->getFrameManager()->getTransform(msg->cloud_curr.header, position_e, orientation_e)) {
        ROS_ERROR("Error transforming from frame '%s' to frame '%s'", msg->cloud_curr.header.frame_id.c_str(),
                qPrintable(context_->getFixedFrame()));
        return;
    }
    ...
    auto points_e = transform_pointcloud(msg->cloud_curr, transform_e, msg->index_curr);
